...
class Modeler:
    ...
    def forecast(data: pd.DataFrame) -> pd.DataFrame:
        ...
        train_data, test_data = Modeler.train_test_split(data=data, date_column='date', n_test=n_test)
        ...
        with mlflow.start_run(experiment_id=experiment.experiment_id, run_name=run_name) as run:
            ...
            # Init prophet model
            krns_prophet = KRNSProphet(date_column='date', key_column='pdr', metric_column='volume_giorno', model=None)
            ...
            print("Log the model")
            signature = infer_signature(train_data, krns_prophet.model.predict(
            ...
            trained_model_rmse = Modeler.evaluate_model(data=trained_model_test_pred, metric='rmse',
            ...
            if prod_model_rmse > trained_model_rmse:
                ...
                # Register model
                model_version = ml_flower.register_model(model_uri=f"runs:/{run.info.run_uuid}/model",
                                                         model_name=pdr_code, timeout_s=10,
                                                         model_flavor_tag=model_flavor)

                # Unit test the model
                unit_test_status = ml_flower.unit_test_model(model_version=model_version, unit_test_days=unit_test_days)
