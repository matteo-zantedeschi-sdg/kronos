...
class Modeler:
    ...
    def train_test_split(data: pd.DataFrame, date_col: str, n_test: int):
        ...
        if data.shape[0] - n_test < n_test:
            logger.warning(
                f"Not enough records to perform train/test split: {data.shape[0]} rows, {n_test} for test"
            )
            raise Exception(
                f"Not enough records to perform train/test split: {data.shape[0]} rows, {n_test} for test"
            )
        else:
            train_data = data.sort_values(by=[date_col], ascending=False).iloc[
                n_test:, :
            ]
            test_data = data.sort_values(by=[date_col], ascending=False).iloc[
                :n_test, :
            ]
